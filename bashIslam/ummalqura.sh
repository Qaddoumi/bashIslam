...
UmmAlQura() {
    ...
    local table_str="${UMMALQURA_TABLE[*]}"
    local table_len="${UMMALQURA_TABLE_LEN}"
...
# Get Gregorian date from UmmAlQura output
get_greg_date() {
    UmmAlQura "$1" "$2" "$3" | grep "^GREG_DATE:" | awk '{print $2, $3, $4}'
}

# Get weekday from UmmAlQura output
get_week_day() {
    UmmAlQura "$1" "$2" "$3" | grep "^WEEK_DAY:" | awk '{print $2}'
}

# Get Julian day from UmmAlQura output
get_julian_day() {
    UmmAlQura "$1" "$2" "$3" | grep "^JULIAN_DAY:" | awk '{print $2}'
}

# Get Hijri date from UmmAlQura output
get_hijri_date() {
    UmmAlQura "$1" "$2" "$3" | grep "^HIJRI_DATE:" | awk '{print $2, $3, $4}'
}

# Get solar Hijri date from UmmAlQura output
get_solar_hijri_date() {
    UmmAlQura "$1" "$2" "$3" | grep "^SOLAR_HIJRI_DATE:" | awk '{print $2, $3, $4}'
}

# Get Islamic lunation number from UmmAlQura output
get_islamic_lunation_num() {
    UmmAlQura "$1" "$2" "$3" | grep "^ISLAMIC_LUNATION_NUM:" | awk '{print $2}'
}

# Get Islamic month length from UmmAlQura output
get_islamic_month_length() {
    UmmAlQura "$1" "$2" "$3" | grep "^ISLAMIC_MONTH_LENGTH:" | awk '{print $2}'
}
